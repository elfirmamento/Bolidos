...
def detect_meteors(video_path, output_folder, speed_factor=1, brightness_threshold=200, min_line_length=50, max_line_gap=10, trail_length=10, motion_threshold=30, curvature_threshold=0.1, velocity_threshold=100, brightness_delta_threshold=50):
                ...
                if curvature < curvature_threshold and line_velocity > velocity_threshold:
                    ...
                    if np.sum(roi) > 0:
                        ...
                        if brightness_delta > brightness_delta_threshold:
                            # Recortar el evento y clasificarlo usando YOLOv5
                            event_image = frame[max(0, center_y - 50):min(frame.shape[0], center_y + 50),
                                               max(0, center_x - 50):min(frame.shape[1], center_x + 50)]

                            results = model(Image.fromarray(event_image))

                            if len(results.xyxy[0]) > 0 and results.xyxy[0][0][5].item() > 0.5:
                                event = {
                                    'frame': frame_count,
                                    ...
                                    'classification': results.xyxy[0][0][6].item()
                                }
                                events.append(event)
    ...
    with open(f"{output_folder}/events_info.txt", "w") as f:
        for event in events:
            f.write(f"Frame: {event['frame']}, Time: {event['time']:.2f}s, Area: {event['area']:.2f}, Position: {event['position']}, Curvature: {event['curvature']:.2f}, Velocity: {event['velocity']:.2f}, Brightness Delta: {event['brightness_delta']}, Classification: {event['classification']}\n")
